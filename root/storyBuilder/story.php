...
<html>
	...
	<body> 
        ...
        <p>The <span <? echo($row["Character.Name"]) ?> ></span> learned of the <span <? echo($row["Villain.Name"]) ?> ></span>’s presence in the country and set off on a journey to to defeat the <span <? echo($row["Villain.Name"]) ?> ></span>.  Navigating to the <span <? echo($row["Villain.Name"]) ?> ></span>’s lair was fairly simple and before long the <span <? echo($row["Character.Name"]) ?> ></span> arrived at the <span <? echo($row["Villain.Lair"]) ?> ></span> that the <span <? echo($row["Villain.Name"]) ?> ></span> called home.  Once there, the <span <? echo($row["Character.Name"]) ?> ></span> <span <? echo($row["Conflict.Fight"]) ?> ></span>.  <span <? echo($row["Character.Name"]) ?> ></span> then <span <? echo($row["Conflict.Ending"]) ?> ></span>.  And they all lived happily ever after!</p>
